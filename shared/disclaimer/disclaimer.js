...
(function () {
  ...
  const TONES = {
    soft: {
      ...
      icon: 'ℹ️',
      text: (cfg) =>
        `本サイトは ${cfg.siteName} の参考情報サービスです。掲載内容は気象・潮汐の数値計算結果をもとにした参考値であり、現地の状況と異なる場合があります。` +
        (cfg.showWeatherLaw !== false ? '気象業務法第17条に基づく気象予報業ではありません。' : '') +
        '本情報を利用した行動による損害について責任を負いません。',
    },
    strict: {
      label: '免責事項',
      color: '#fbbf24',
      icon: '⚠️',
      text: (cfg) =>
        `【免責事項】${cfg.siteName} の掲載情報は気象・潮汐の数値計算結果をもとに算出した参考値であり、正確性を保証しません。` +
        (cfg.showWeatherLaw !== false ? '気象業務法第17条に基づく気象予報業ではありません。' : '') +
        '自然環境・気象条件は急変します。現地の状況・安全を必ずご自身で確認してください。' +
    ...
    warn: {
      ...
      icon: '🚨',
      text: (cfg) =>
        `【重要】${cfg.siteName} の情報は数値計算結果をもとにした参考値です。荒天・強風時は現地への訪問を中止してください。` +
